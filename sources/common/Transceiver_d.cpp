...
void Transceiver::Send(const uint8 *data, uint size)
{
    ...
    for (uint i = 0; i < size; i++)
    {
        ...
        HAL_PIO::Set(PIN_MODE1);                     // Установить MODE1 в "1" - это означает, что M0M1 == 01 и устройство ждёт подверждения от панели о принятых данных

        while(HAL_PIO::Read(PIN_READY) == 0) {};     // Ожидаем сигнал подтверждения - "1" на READY будет означать, что панель приняла данные //-V712

        HAL_PIO::Reset(PIN_MODE1);                   // Установить MODE1 в "0" - это означает, что устройство в состоянии Disable

        while(HAL_PIO::Read(PIN_READY) == 1) {};         // Ожидаем, когда уровень на READY станет раным "0". //-V712
    }
