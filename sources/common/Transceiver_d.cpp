...
void Transceiver::Send(const uint8 *data, uint size)
{
    ...
    for (uint i = 0; i < size; i++)
    {
        ...
        GPIOD->BSRR = GPIO_PIN_4;                       // Установить MODE1 в "1" - это означает, что M0M1 == 01 и устройство ждёт подверждения от панели о принятых данных

        while (!(GPIOC->IDR & GPIO_PIN_4)) {};      // Ожидаем сигнал подтверждения - "1" на READY будет означать, что панель приняла данные //-V712

        GPIOD->BSRR = (uint)GPIO_PIN_4 << 16U;          // Установить MODE1 в "0" - это означает, что устройство в состоянии Disable

        while (GPIOC->IDR & GPIO_PIN_4) {};         // Ожидаем, когда уровень на READY станет раным "0". //-V712
    }
