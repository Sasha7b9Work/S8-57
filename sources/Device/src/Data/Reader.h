...
extern uint8 *dataIN[Chan::Number];  ///< Считанные данные первого канала
extern uint8 *dataOUT[Chan::Number]; ///< Считанные данные второго канала
...
typedef struct
{
    uint8       data[Chan::Number][281 * 2];    ///< Данные обоих каналов. Точек в два раза больше, чем на экране, для пикового детектора
    bool        needDraw[Chan::Number];         ///< Если true, то канал 1 надо рисовать
    uint8       notUsed0[2];
