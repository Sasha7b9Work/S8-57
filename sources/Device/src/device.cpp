...
void Device::Init()
{
    Hardware::Init();               // Инициализация аппаратной части производится в самом начале.

    PAUSE_ON_MS(500);               // задержка нужна, чтобы инициализировалась панель

    VCP::Init();                    // Инициализация VCP производится как можно раньше, чтобы работал отладочный лог
