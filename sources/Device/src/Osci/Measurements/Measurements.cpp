...
int CalculatePeriodAccurately(Chan::E ch)
{
    ...
    if(!periodAccurateIsCalculating[ch])
    {
        ...
        {
            uint8 point = *data++;
            if(point < _FPGA::VALUE::MIN || point >= _FPGA::VALUE::MAX)
            {
                EXIT_FROM_PERIOD_ACCURACY
            }
            *pSum = *(pSum - 1) + point;
...
float Measure::CalculateCursorU(Chan::E ch, float posCurT)
{
    ...
    int rel = (int)(CHOICE_BUFFER)[(int)points.word0 + ROUND(int, posCurT)] - _FPGA::VALUE::MIN;

#define SCALE (200.0F / (_FPGA::VALUE::MAX - _FPGA::VALUE::MIN))
...
#define LAST_POINT  (points.sword1)
    ...
    int prevData = 200 - dataIn[FIRST_POINT] + _FPGA::VALUE::MIN;

    int numIntersections = 0;

    for(int i = FIRST_POINT + 1; i < LAST_POINT; i++)
    {
        int curData = 200 - (dataIn)[i] + _FPGA::VALUE::MIN;
...
void MeasurementsOsci::SetData()
{
    ...
    if (isSet)
    {
        ...
        if (TBASE_DS >= TBase::MIN_P2P)             // Если находимся в поточечном режме, то нужно брать последние считанные точки для проведения измерений
        {
            ...
            {
                if (IN_A[i] != _FPGA::VALUE::NONE)                // Если это значение считано
                {
                    lastByte = i;
...
static void LimitationData(Chan::E ch, uint numBytes)
{
    ...
    {
        Math::Limitation<uint8>(data, _FPGA::VALUE::MIN, _FPGA::VALUE::MAX);
        data++;
    }
}
