...
static void WriteTextVoltage(Chan::E ch, int x, int y)
{
    ...
    char buffer[SIZE];
    std::snprintf(buffer, SIZE, "%s\xa5%s\xa5%s", (ch == ChanA) ? "1к" : "2к", ModeCouple::UGO(set.ch[ch].couple), Range::ToString(ch, DIVIDER(ch)));
    String(buffer).Draw(x + 1, y, colorDraw);

    char bufferTemp[SIZE];
    std::snprintf(bufferTemp, SIZE, "\xa5%s", RShift::ToString(set.ch[ch].rShift, range, DIVIDER(ch)).c_str());
