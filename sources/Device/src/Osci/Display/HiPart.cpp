...
static void WriteTextVoltage(Chan::E ch, int x, int y)
{
    ...
    std::snprintf(bufferTemp, SIZE, "\xa5%s", RShift::ToString(RShift::Value(ch), range, static_cast<int8>(set.ch[ch].divider)).CString());
...
static void WriteCursors()
{
    ...
    if (CursorsMeasurements::NecessaryDraw())
    {
        ...
        if (set.curs.cntrlU[set.curs.source] == CursorsControl::Disable)
        {
            ...
            x = 49;
            float pos0 = MathFPGA::VoltageCursor(CursorsMeasurements::PosU(source, 0), set.ch[source].range, RShift::Value(source));
            float pos1 = MathFPGA::VoltageCursor(CursorsMeasurements::PosU(source, 1), set.ch[source].range, RShift::Value(source));
