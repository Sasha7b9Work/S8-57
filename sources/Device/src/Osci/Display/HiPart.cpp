...
static void WriteTextVoltage(Chan::E ch, int x, int y)
{
    ...
    char buffer[SIZE];
    std::snprintf(buffer, SIZE, "%s\xa5%s\xa5%s", Chan(ch).IsA() ? "1к" : "2к", ModeCouple(SET_COUPLE(ch)).UGO(), Range(range).ToString(divider[ch]));
    String(buffer).Draw(x + 1, y, colorDraw);

    char bufferTemp[SIZE];
    std::snprintf(bufferTemp, SIZE, "\xa5%s", RShift::ToString((uint16)SET_RSHIFT(ch), range, divider[ch]).CString());
