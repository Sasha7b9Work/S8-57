...
static void WriteTextVoltage(Chan::E ch, int x, int y)
{
    ...
    char buffer[SIZE];
    std::snprintf(buffer, SIZE, "%s\xa5%s\xa5%s", (ch == ChanA) ? "1к" : "2к", ModeCouple::UGO(set.ch[ch].couple), Range::ToString(ch, static_cast<int8>(set.ch[ch].divider)));
    String(buffer).Draw(x + 1, y, colorDraw);

    char bufferTemp[SIZE];
    std::snprintf(bufferTemp, SIZE, "\xa5%s", RShift::ToString(set.ch[ch].rShift, range, static_cast<int8>(set.ch[ch].divider)).c_str());
...
static void WriteCursors()
{
    if (CursorsMeasurements::NecessaryDraw())
    {
        ...
        if (!CursorsControl::IsDisabledU())
        {
            ...
            float delta = std::fabsf(pos1 - pos0) * Divider::ToAbs(set.ch[source].divider);
            String(":dU=").Draw(x, y1);
