...
static int DrawMainParameters(int _x, int _y)
{
    ...
    */
    ...
    if (set.mem.modeWork == ModeWork::Dir)
    {
        ...
        std::snprintf(buffer, 100, "с\xa5\x10%s", source[static_cast<uint8>(set.trig.source)]);
    }
...
static void WriteTextVoltage(Chan::E ch, int x, int y)
{
    ...
    char buffer[SIZE];
    std::snprintf(buffer, SIZE, "%s\xa5%s\xa5%s", Chan(ch).IsA() ? "1к" : "2к", ModeCouple(ch).UGO(), Range(range).ToString(static_cast<int8>(Divider(ch))));
