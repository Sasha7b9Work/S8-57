...
static int DrawMainParameters(int _x, int _y)
{
    ...
    */
    ...
    std::snprintf(buffer, SIZE, "р\xa5%s", TBase().ToString());
    ...
    if (ModeWork::IsDir())
    {
        ...
        std::snprintf(buffer, 100, "с\xa5\x10%s", source[TrigSource()]);
    }
    ...
    if (ModeWork::IsDir())
    {
        ...
        Char(filtr[TrigInput()][1]).Draw(x + 53, y1);
    }
    ...
    if (ModeWork::IsDir())
    {
        std::snprintf(buffer, 100, "\xa5\x10%c", mode[TrigStartMode()]);
...
static void WriteTextVoltage(Chan::E ch, int x, int y)
{
    ...
    char buffer[SIZE];
    std::snprintf(buffer, SIZE, "%s\xa5%s\xa5%s", Chan(ch).IsA() ? "1к" : "2к", ModeCouple(ch).UGO(), Range(ch).ToString(static_cast<int8>(Divider(ch))));
    String(buffer).Draw(x + 1, y, colorDraw);

    char bufferTemp[SIZE];
    std::snprintf(bufferTemp, SIZE, "\xa5%s", RShift::ToString(RShift(ch), range, static_cast<int8>(Divider(ch))).c_str());
...
static void WriteCursors()
{
    if (CursorsMeasurements::NecessaryDraw())
    {
        ...
        if (CursorsControl::IsDisabledT())
        {
            ...
            float pos0 = MathFPGA::TimeCursor(CursorsMeasurements::PosT(source, 0), TBase());
            float pos1 = MathFPGA::TimeCursor(CursorsMeasurements::PosT(source, 1), TBase());
