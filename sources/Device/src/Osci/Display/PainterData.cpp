...
static void DrawChannel(Chan::E ch)
{
    ...
    float scale = (float)Grid::Height() / (_FPGA::VALUE::MAX - _FPGA::VALUE::MIN);
...
static void DrawModeLinesPeakDetOn(int center, const uint8 *data, float scale, int x)
{
    ...
    {
        int min = (int)(center - (data[i] - _FPGA::VALUE::AVE) * scale + 0.5F);
        int max = (int)(center - (data[i + 1] - _FPGA::VALUE::AVE) * scale + 0.5F);
...
static void DrawModeLinesPeakDetOff(int center, const uint8 *data, float scale, int x)
{
    ...
    for (int i = 1; i < 281; i++)
    {
        int value = (int)(center - (data[i] - _FPGA::VALUE::AVE) * scale + 0.5F);
        int valuePrev = (int)(center - (data[i - 1] - _FPGA::VALUE::AVE) * scale + 0.5F);
        ...
        if (coordVert == -1 && data[i] == _FPGA::VALUE::NONE)
        {
            coordVert = i;
...
static void DrawModePointsPeakDetOn(int center, const uint8 *data, float scale, int x)
{
    for (int i = 0; i < 281 * 2; i += 2)
    {
        Point().Draw(x, (int)(center - (data[i] - _FPGA::VALUE::AVE) * scale + 0.5F));
        Point().Draw(x, (int)(center - (data[i + 1] - _FPGA::VALUE::AVE) * scale + 0.5F));
        x++;
    }
}
...
static void DrawModePointsPeakDetOff(int center, const uint8 *data, float scale, int x)
{
    for (int i = 0; i < 280; i++)
    {
        float value = center - (data[i] - _FPGA::VALUE::AVE) * scale;
