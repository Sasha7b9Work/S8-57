...
#define MAX_NUM_SAVED_WAVES 23  ///< \brief „исло сохран€емых во внутреннем ѕѕ«” измерений. ѕока ограничено количеством квадратиков, которые можно 
                                ///< вывести в одну линию внизу сетки.


/// —труктура описывает адрес в адресном пространстве контроллера
struct Address
{
    uint addressMC;
    /// ¬озвращает адрес из адресного пространства контроллера, рассчитанный из адреса PC
    static Address FromPC(uint addr);
    static Address FromPC(uint8 *addr);
    /// ‘ормируте адрес из адресного пространства микроконтроллера
    static Address FromMC(uint addr);
    /// ѕреобразует в адрес PC
    uint8 *ToPC() const;
private:
