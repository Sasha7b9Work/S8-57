...
const PacketROM *Sector::WriteData(uint numInROM, const DataSettings *ds) const
{
    ...
    if (number == 21 && needLog)
    {
        ...
        LOG_WRITE("Записал данные %2d(%d/%d) в  сектор  %d 0x%x, пакет 0x%x, смещ. от нач. сект. 0x%x/%d",
    ...
    if (ROM::needLog)
    {
        LOG_WRITE("Записал данные %2d в сектор  %d 0x%x адрес пакета 0x%x", numInROM, number, address, packet);
        LOG_WRITE("Памяти нужно %d, от конца сектора находится на %d", PacketROM::GetPackedSize(ds), static_cast<int>(End()) - reinterpret_cast<int>(packet));
        ds->Log();
    }
...
const DataSettings *Sector::ReadData(uint numInROM) const
{
    ...
    if (packet)
    {
        ...
        if (ROM::needLog)
        {
            LOG_WRITE("");
            uint offset = reinterpret_cast<uint>(packet) - address;
            LOG_WRITE("Считал данные %2d из сектора %d 0x%x адрес пакета 0x%x, смещ. от нач. сект. 0x%x/%d", numInROM, number, address, packet, offset, offset);
            LOG_WRITE("Памяти нужно %d, от конца сектора находится на %d", PacketROM::GetPackedSize(ds), static_cast<int>(End()) - reinterpret_cast<int>(packet));
            ds->Log();
        }
...
const PacketROM *Sector::DeleteData(uint numInROM) const
{
    ...
    if (packet)
    {
        if (number == 21 && needLog)
        {
            LOG_WRITE("Стираю  данные %2d из сектора %d 0x%x, пакет 0x%x", numInROM, number, address, packet);
        }
