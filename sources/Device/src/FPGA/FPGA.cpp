...
{
    static uint timeFirstZero[Chan::Number] = { 0, 0 };          // В этот момент смещение первый раз достигло 0
    ...
    if (SET_RSHIFT(ch) == RShift::ZERO &&   // Если смещение == 0
        timeFirstZero[ch] == 0)                 // и оно ещё не было достигнуто
    {
        timeFirstZero[ch] = TIME_MS;            // запоминаем время достижения нуля
    }
